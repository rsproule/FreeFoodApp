...
//      print( int.parse(snapshot.value['numStars']));
  ...
  Widget build(BuildContext context) {
    String startTime = snapshot.value['start_time'];
    startTime = startTime.replaceAll("p.m.", "PM");
    startTime = startTime.replaceAll("a.m.", "AM");
    String endTime = snapshot.value['end_time'];
    endTime = endTime.replaceAll("p.m.", "PM");
    endTime = endTime.replaceAll("a.m.", "AM");
...
//                      new Icon(Icons.info_outline),
  ...
  _convertTimeInfoToDateTimeObj(String rawDate, String startTime) {
    ...
    ;
    if (startTime.endsWith("p.m.") || startTime.endsWith("PM")) {
      hour += 12;
    }
